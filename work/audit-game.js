...
requirePattern("follow-up owner stable identity", /let followupOwnerPlayerId=null/);
requirePattern("follow-up owner captured from active player", /followupOwnerPlayerId=players\[activePlayer\]\?\.playerId/);
requirePattern("follow-up owner included in snapshots", /followupOwnerPlayerId,pack:/);
requirePattern("follow-up remote authorization by identity", /entry\.playerId===followupOwnerPlayerId/);
requirePattern("follow-up stale epoch/key guard", /msg\.gameEpoch===gameEpoch&&msg\.questionKey===current\?\.key/);
requirePattern("follow-up passive phone state", /phoneFollowWait|phoneFollowOfferWait/);
requirePattern("follow-up phone controls gated", /phoneOwnsFollowup\(\)/);
