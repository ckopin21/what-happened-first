...
const errors=[],seenIds=new Set(),seenClues=new Set(),historyQuestions=[];
...
function checkClue(file,clue,index,kind){
  ...
  const repeated=historyQuestions.find(record=>record.id!==clue.id&&normalize(record.question)===normalize(clue.question));if(repeated&&!clue.allowReuse)fail(file,`${label} repeats permanent question '${repeated.id}' without allowReuse:true`);
  ...
  if(clue.followup){const followup=clue.followup;if(!text(followup.id)||!text(followup.question)||!text(followup.answer)||!text(followup.hint)||!Array.isArray(followup.aliases)||!followup.aliases.length)fail(file,`${label}.followup is incomplete`);if(kind==="timeline"&&(!Array.isArray(followup.choices)||followup.choices.length!==4||!text(followup.correctChoice)||!followup.choices.includes(followup.correctChoice)))fail(file,`${label}.followup needs four choices and a matching correctChoice`);if(seenClues.has(followup.id))fail(file,`${label}.followup.id '${followup.id}' is globally duplicated`);seenClues.add(followup.id)}
}
